...
def index(request):
    if request.method != "POST":
        ...
        for question in latest_question_list:
            if question.tema not in listaTemas:
                listaTemas.append(question.tema)
        ...
        latest_question_list = Question.objects.order_by('-pub_date')
        resultado = filter(lambda preg:preg.tema == request.POST["listTemas"], latest_question_list)
        template = loader.get_template('polls/index.html')
        context = {
                    'title':'Lista de preguntas de la encuesta',
                    'latest_question_list': resultado
...
def choice_add(request, question_id):
        ...
        if question.choice_set.count() == 4:
            return render(request, 'polls/choice_new.html', {'title':'Ya tiene todas las opciones'})
        if request.method =='POST':
